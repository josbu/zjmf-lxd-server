...
func StartNATSyncService() {
	log.Println("✓ NAT规则同步服务启动")

	time.Sleep(15 * time.Second)
	SyncAllNodesNATAsync()
...
func SyncAllNodesNATAsync() {
	...
	log.Printf("[NAT-SYNC] 开始同步 %d 个活动节点的NAT规则", len(nodes))
	
	for _, node := range nodes {
		go SyncNodeNATRules(node.ID, false)
	}
}
...
func SyncNodeNATRules(nodeID uint, manual bool) error {
	...
	log.Printf("[NAT-SYNC] 开始同步节点 %s (ID: %d)%s NAT规则", node.Name, node.ID, map[bool]string{true: " [手动]", false: ""}[manual])

	result := callNodeAPIForNAT(node, "GET", "/api/nat/list", nil)
	if result["code"] != float64(200) {
		task.Status = "failed"
		task.ErrorMessage = fmt.Sprintf("获取NAT规则列表失败: %v", result["msg"])
		endTime := time.Now()
		task.EndTime = &endTime
		database.DB.Save(&task)

		log.Printf("[NAT-SYNC] 节点 %s 连接失败，清理旧NAT规则缓存", node.Name)
		database.DB.Unscoped().Where("node_id = ?", node.ID).Delete(&models.NATRuleCache{})
		
		return fmt.Errorf("获取NAT规则列表失败")
	}
	...
	if !ok {
		...
		log.Printf("[NAT-SYNC] 节点 %s 返回数据格式错误，清理旧NAT规则缓存", node.Name)
		database.DB.Unscoped().Where("node_id = ?", node.ID).Delete(&models.NATRuleCache{})
	...
	for _, item := range data {
		...
		if err := updateNATCache(node, ruleData); err != nil {
			log.Printf("[NAT-SYNC] 更新NAT规则缓存失败: %v", err)
			failedCount++
		} else {
			successCount++
	...
	log.Printf("[NAT-SYNC] 节点 %s NAT规则同步完成: 成功 %d, 失败 %d, 总计 %d", 
